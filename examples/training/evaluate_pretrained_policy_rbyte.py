...
def _eval_yaak(hydra_cfg: DictConfig) -> None:
    ...
    output_dir = (
        ...
        / "v044" / Path(hydra_cfg.model.artifact).name
        / "same_noise"
    )
    ...
    policy, train_cfg, preprocessor, postprocessor = instantiate(hydra_cfg.model)
    predict_main_yaak(
        policy, dataloader_test, train_cfg, output_dir, preprocessor=preprocessor, postprocessor=postprocessor
    )
